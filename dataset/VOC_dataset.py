...
def flip(img, boxes):
    img = img.transpose(Image.FLIP_LEFT_RIGHT)   #水平翻转每一行的数据都倒叙
    w = img.width
    if boxes.shape[0] != 0:
        xmin = w - boxes[:,2]
        xmax = w - boxes[:,0]
...
class VOCDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,root_dir,resize_size=[640,800],split='trainval',use_difficult=False,is_train=True, augment=None, mean=[0.5,0.5,0.5], std=[1.,1.,1.]):
        ...
        初始化主要的工作：
        1.告知文件path
        2.self.img_ids 图片名称列表
        3.self.name2id 标签类别到整数的映射字典
        """
        self.root=root_dir
        ...
        self.img_ids=[x.strip() for x in self.img_ids]   #img_id
        self.name2id=dict(zip(VOCDataset.CLASSES_NAME,range(len(VOCDataset.CLASSES_NAME))))  #类别映射成字典
        self.id2name = {v:k for k,v in self.name2id.items()}      #类别和类别索引换位置
    ...
    def preprocess_img_boxes(self,image,boxes,input_ksize):
        ...
        scale=min_side/smallest_side   #缩放最小边/图片最小边
        ...
        pad_h=32-nh%32
        #生成一张空白的图像
        image_paded = np.zeros(shape=[nh+pad_h, nw+pad_w, 3],dtype=np.uint8)   #其余位置补充0
    ...
    def collate_fn(self,data):
        # batch中的图片大小pad一致，每个label里面的object都填充成最多的objects图片的label长度
        ...
        # classes pad
        for i in range(batch_size):
            pad_boxes_list.append(torch.nn.functional.pad(boxes_list[i],(0,0,0,max_num-boxes_list[i].shape[0]),value=-1))   #function.pad 填充
