...
from tools.common_tools import make_logger
...
def main(opt):
    # log
    log_path = r"D:/datasets/KITTI_VOC2007/results"
    logger, log_dir = make_logger(log_path)
    ...
    train_dataset = VOCDataset(root_dir=opt['data_root_dir'], resize_size=[640,800], split='trainval',use_difficult=False,is_train=True,augment=transform, mean=config.mean,std=config.std)

    model = FCOSDetector(mode="training").cuda()
    model = torch.nn.DataParallel(model)   #多个gpu加速训练
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True,
                                            ...
                                            num_workers=opt['n_cpu'], worker_init_fn=np.random.seed(0))   #pad batch object长度为一样
    ...
    model.train()   #为啥要在这里面train，不加也可以
    # print(model)
    logger.info(model)
    ...
    for epoch in range(EPOCHS):
        for epoch_step, data in enumerate(train_loader):
            ...
            if GLOBAL_STEPS%50 == 0:
                ...
                logger.info("global_steps:%d epoch:%d steps:%d/%d cls_loss:%.4f cnt_loss:%.4f reg_loss:%.4f cost_time:%dms lr=%.4e total_loss:%.4f" % \
                    (GLOBAL_STEPS, epoch + 1, epoch_step + 1, steps_per_epoch, losses[0].mean(), losses[1].mean(),
                    losses[2].mean(), cost_time, lr, loss.mean()))
            GLOBAL_STEPS += 1
